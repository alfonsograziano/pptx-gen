...
See [`examples/workspace/projects/how-it-works/`](examples/workspace/projects/how-it-works).</sup>
...
- **Design from scratch.** Draw slides from native shapes, text, lines, and vector
  icons, guided by a customizable [design system](starter/design.md). No template needed.
...
- a customizable [design system](starter/design.md),
...
native shapes, text, and icons, and the [design system](starter/design.md) supplies the
...
hand-build. Your workspace's `design.yml` is the single place that steers how
...
Two things live in two places: the **engine** (this repo) and your **workspace**
(your brand, templates and decks). Keeping them apart is the point — you can
update the engine without carrying your decks along, or touching them at all.
...
`npm link` puts `pptx-gen` on your PATH while still pointing at this clone, so a
later pull updates the command in place. Then create a workspace, anywhere you
like, and hand the skills to your agent:
...
`init` creates this, and nothing in it depends on where the engine lives:
...
Updating the engine is then just a pull in the clone. Your workspace is
untouched, and a `design.yml` written before a new token existed keeps working,
because anything it omits falls back to the engine default.

Want a separate brand per client? Run `init` again somewhere else. Commands find
the workspace by walking up from wherever you are, so `cd ~/clients/acme && pptx-gen …`
just works; `--workspace <dir>` or `PPTX_GEN_WORKSPACE` override it.

The `npx skills add` step uses [skills.sh](https://skills.sh) to install the four
skills into your agent's skills directory (for Claude Code, `.claude/skills/`), so
it discovers them automatically. Prefer to wire them in by hand? See [Skills](#skills).

## Quick start

You never write build code. Once the skills are installed, you just talk to your
coding agent from inside your workspace, in plain language. Each kind of request
maps to one of four skills, and the agent runs the whole workflow — writing the
script, running the CLI, reviewing the rendered screenshots, fixing issues, and
handing back the files.
...
The agent reads your workspace's `design.md` for the visual language, picks
templates where one fits or designs slides from scratch where none does, writes a
build script (`~/decks/projects/payments-api/build.ts`), builds it, inspects the
screenshots and build report, and hands back
`~/decks/projects/payments-api/output/deck.pptx`. To change anything, just say so
— *"make slide 3 a two-column layout and cut the jargon"* — and it edits the
script and rebuilds.
...
Each imported slide becomes a folder under your workspace's `templates/` with its
...
The agent edits your workspace's `design.yml` and `design.md` together so the
values and their documentation stay in sync. Both are yours — an engine update
...
A template is a folder under your workspace's `templates/`. Each holds one slide:

```text
<workspace>/templates/title-cover/
...
```bash
pptx-gen workspace                  # where everything lives
npm run show-all-templates          # builds a preview folder of screenshots
find "$(pptx-gen workspace --json | jq -r .templates)" -maxdepth 2 -name description.md | sort
```
...
Colours, fonts, the slide grid, and optional logos live in one place: `design.yml`
in your workspace, documented in its `design.md`. This is the visual language
every scratch-built slide follows: the helpers and design tokens read straight
from it, so a card's accent bar, the body font, and the footer all come from the
design. **Change `design.yml` and every scratch-built slide restyles at once** —
no need to touch each slide.

The file is a partial override of the engine's defaults
([`src/design.ts`](src/design.ts)): anything you leave out falls back, so a brand
file keeps working across engine updates. It lives in your workspace, so an
...
pptx-gen ingest --source "deck.pptx" --template two-column --slide 3

# every slide as its own template
pptx-gen ingest --source "deck.pptx" --template mydeck --split
```

Templates land in the workspace's `templates/`.
...
```bash
pptx-gen init [dir]             # create a workspace
pptx-gen workspace [--json]     # show where everything resolves to
...
Every command takes `--workspace <dir>`; without it, the workspace is found by
walking up from where you are, then `$PPTX_GEN_WORKSPACE`, then `~/.pptx-gen`.
...
- **`pptx-deck`** — turn a brief or notes into a finished deck: read the design,
  ...
  `pptx-gen build`, review screenshots, and return the `.pptx`.
- **`ingest-slide-templates`** — import slides from a `.pptx` into the library
  (`pptx-gen ingest`) and document each one.
